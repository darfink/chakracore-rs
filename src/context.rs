...
impl Context {
    ...
    pub unsafe fn from_object(object: &JsValueRef) -> Result<Context> {
        let mut reference = JsContextRef::new();
        jstry!(JsGetContextOfObject(object.clone(), &mut reference));
        Ok(Context::from_raw(reference))
    }
...
impl<'a> ContextGuard<'a> {
    ...
    pub fn global(&self) -> Result<value::Value> {
        let mut value = JsValueRef::new();
        unsafe {
            jstry!({ JsGetGlobalObject(&mut value) });
            Ok(value::Value::from_raw(value))
        }
    }
}
