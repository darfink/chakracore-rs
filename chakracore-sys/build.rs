...
const LIBS: [(&'static str, &'static str); 1] = [("lib", "ChakraCoreStatic")];
...
fn main() {
  ...
  assert!(
    overrides.iter().filter(|var| var.is_ok()).count() != 1,
...
fn setup_custom() -> (PathBuf, Vec<PathBuf>) {
  ...
  (
    src_dir,
    LIBS.iter().map(|&(dir, _)| build_dir.join(dir)).collect(),
  )
}
...
fn setup_default() -> (PathBuf, Vec<PathBuf>) {
  ...
  let src_dir =
    PathBuf::from(&get!("CARGO_MANIFEST_DIR")).join(format!("target/source-{}", VERSION));
  ...
  let has_required_libs = if cfg!(feature = "static") {
    ...
    LIBS
      .iter()
      .all(|&(_, name)| lib_dir.join(linking::format_lib(name)).exists())
  } else {
...
mod build {
  ...
  pub fn copy_libs(build_dir: &Path, lib_dir: &Path) {
    ...
    for dependency in deps {
      ...
      fs::copy(&dependency, lib_dir.join(file_name)).expect(&format!(
        "Failed to copy '{:?}' to target directory",
        dependency
      ));
    }
  }
...
mod linking {
  ...
  pub fn setup(search_paths: &[PathBuf]) {
    ...
    if cfg!(feature = "static") {
      ...
      link_manually(
        "static",
        &LIBS.iter().map(|&(_, name)| name).collect::<Vec<_>>(),
      );
...
mod binding {
  ...
  pub fn generate(src_dir: &Path) {
    ...
    if util::has_target("windows") {
      ...
      builder = ["-include", "stdint.h", "-Wno-pragma-once-outside-header"]
        .iter()
        .fold(builder, |builder, carg| builder.clang_arg(*carg));
    }
  ...
  fn sanitize_interface(mut content: String) -> String {
    ...
    regex_replace(
      &mut content,
      r"pub type (?P<name>\w+).+(?P<type>\*mut.+c_void);",
      &[
        "#[repr(C)]",
